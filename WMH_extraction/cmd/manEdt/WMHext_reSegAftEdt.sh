...
reSegAftEdt(){
	...
	ID="$1"
	echo "UBO Detector: Re-segmenting WMH into regions (ID = ${ID}) ..."
	...
	## pipeline path
	pipelinePath=$(dirname $(dirname $(dirname $0)))

	## PVWMH magnitude
	PVmag=$3

	## Probability threshold
	# probThr_str=`echo $4 | sed 's/\./_/g'`
	...
	# --------------------------------------------------
	# No. of WMH clusters (meaning No. of voxels) in lobar segments and globally
	# --------------------------------------------------
